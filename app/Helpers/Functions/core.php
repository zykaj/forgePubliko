...
function getViewContent($view, $data = [], $mergeData = [])
{
	if (view()->exists(config('larapen.core.customizedViewPath') . $view)) {
		$view = View::make(config('larapen.core.customizedViewPath') . $view, $data, $mergeData);
	} else {
...
function isFromApi()
{
	...
	if (
		...
		&& Str::contains(Route::currentRouteAction(), plugin_namespace('api'))
	) {
...
function isFromAdminPanel($url = null)
{
	if (empty($url)) {
		...
		try {
			...
			$isValid = (Str::startsWith($urlPath, $adminUri)
...
function isDemo($url = null)
{
	if (isDemoDomain()) {
		if (
			Str::contains(Route::currentRouteAction(), 'Post\DetailsController@sendMessage')
			|| Str::contains(Route::currentRouteAction(), 'Post\ReportController@sendReport')
			|| Str::contains(Route::currentRouteAction(), 'PageController@contactPost')
		) {
			return true;
		}
		if (auth()->check()) {
			if (isFromAdminPanel($url)) {
				if (auth()->user()->can(Permission::getStaffPermissions()) && auth()->user()->id == 1) {
					return false;
				}
				...
				if (
					!Str::contains(Route::currentRouteAction(), 'Controllers\HomeController')
					...
					&& !Str::contains(Route::currentRouteAction(), 'Post\CreateOrEdit\MultiSteps\PhotoController')
					&& !Str::contains(Route::currentRouteAction(), 'Post\CreateOrEdit\MultiSteps\PaymentController')
...
function isFromUrlThatCanContainCountryCode()
{
	if (config('settings.seo.multi_countries_urls')) {
		if (
			Str::contains(Route::currentRouteAction(), 'SearchController')
			|| Str::contains(Route::currentRouteAction(), 'CategoryController')
			|| Str::contains(Route::currentRouteAction(), 'CityController')
			|| Str::contains(Route::currentRouteAction(), 'UserController')
			|| Str::contains(Route::currentRouteAction(), 'TagController')
			|| Str::contains(Route::currentRouteAction(), 'CompanyController')
...
{
	if (empty($url)) {
		$isValid = (Str::endsWith(request()->url(), '.xml')
			|| Str::contains(Route::currentRouteAction(), 'SitemapsController'));
	} else {
...
function isAvailableLang($abbr)
{
	...
	$lang = Cache::remember('language.' . $abbr, $cacheExpiration, function () use ($abbr) {
		$lang = Language::firstWhere('abbr', $abbr);
...
function getUrlPageByType($type, $locale = null)
{
	...
	$page = Cache::remember($cacheId, $cacheExpiration, function () use ($type, $locale) {
		return Page::type($type)->transIn($locale)->first();
	});
...
function getMetaTag($tag, $page)
{
	...
	try {
		...
		$metaTag = Cache::remember($cacheId, $cacheExpiration, function () use ($languageCode, $page) {
			return MetaTag::transIn($languageCode)->where('page', $page)->first();
		});
	} catch (\Exception $e) {
	}
