...
trait SettingsTrait
{
	...
	public function applyFrontSettings()
	{
		...
		*/
		...
		Meta::set('description', strip_tags($description));
		...
		try {
			$this->og->siteName(config('settings.app.app_name', 'PublikoAL'))->locale($locale)->type('website')->url(rawurldecode(url()->current()));
		} catch (\Exception $e) {
		};
